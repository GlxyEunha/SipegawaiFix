...
@section('body')
    ...
    <div class="flex justify-between items-center w-full">
        <div class="flex items-left">
            {{-- Search & Filter --}}
            <form id="search-form" method="GET" action="{{ route('pegawai.tugas') }}" class="flex items-center">
                {{-- Search --}}
        ...
        <a href="{{ route('pegawai.riwayatTugas') }}" class="inline-block px-6 py-3 bg-indigo-600 text-white font-medium rounded-md hover:bg-indigo-700">
                        ...
                        <tbody class="bg-white">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 border-b border-gray-200 text-center">
                                    <a href="{{ route('pegawai.editTugas', $t->id_tugas) }}" class="px-3 py-1 bg-blue-500 text-white text-xs font-semibold rounded">Edit</a>
                                    <form action="{{ route('pegawai.destroyTugas', $t->id_tugas) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
